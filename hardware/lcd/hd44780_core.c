...
uint8_t hd44780_flash_printf(const char * format, ...) {
	char display_buf[LCD_CHAR_PER_LINE]; /*!< Pufferstring fuer Displayausgaben */
	va_list	args;

	/* Sicher gehen, dass der zur Verfuegung stehende Puffer nicht ueberlaeuft */
	va_start(args, format);
	const uint8_t len = (uint8_t) vsnprintf_P(display_buf, LCD_CHAR_PER_LINE, format, args);
	va_end(args);
 ...
 * @return Anzahl der geschriebenen Zeichen
 */
uint8_t hd44780_flash_puts(const char * text) {
	const char * ptr = text;
	uint8_t len = 0;
	char tmp;
